// resources/js/Pages/EditHeaderContact.jsx

import { useForm } from "@inertiajs/react";
import Swal from "sweetalert2";
import { Head } from "@inertiajs/react";

const EditHeaderContact = ({ dataHeaderContact }) => {
  // Initialize form with default values
  const { data, setData, put, processing, errors } = useForm({
    title: dataHeaderContact?.title || "",
    description: dataHeaderContact?.description || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!data.image_url) {
      formData.append("existing_image_url", dataHeaderContact?.image_url || "");
    } else {
      formData.append("image_url", data.image_url);
    }

    put(`/header-contact/${dataHeaderContact?.id}`, {
      data: formData,
      ...
      onSuccess: () => {
        Swal.fire({
          ...
          window.location.href = "/header-contact"; // Redirect after success
        });
      },
      onError: () => {
        Swal.fire({
  ...
  return (
    <div className="bg-white p-6">
      ...
      <h1 className="mb-6 text-2xl font-bold">Edit Header Contact</h1>
...
export default EditHeaderContact;
