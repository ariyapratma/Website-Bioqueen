...
const EditHeroMaklonValue = ({ dataHeroMaklonValue }) => {
  const { data, setData, post, processing, errors } = useForm({
    title: dataHeroMaklonValue?.title || "",
    heading1: dataHeroMaklonValue?.heading1 || "",
    content1: dataHeroMaklonValue?.content1 || "",
    image_url1: null,
    heading2: dataHeroMaklonValue?.heading2 || "",
    content2: dataHeroMaklonValue?.content2 || "",
    image_url2: null,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    post(`/hero-maklon-value/${dataHeroMaklonValue.id}`, {
      onSuccess: () => {
        Swal.fire({
          title: "Success!",
          text: "Hero Maklon Value has been updated successfully.",
          icon: "success",
      ...
      onError: () => {
        Swal.fire({
          title: "Error!",
          text: "There was an error updating the Hero Maklon Value.",
          icon: "error",
          confirmButtonText: "OK",
        });
      },
  ...
  return (
    <div className="bg-white p-6">
      <Head title="Edit Hero Maklon Value | PT Ratu Bio Indonesia" />
      <h1 className="mb-6 text-2xl font-bold">Edit Hero Maklon Value</h1>
...
export default EditHeroMaklonValue;
