// resources/js/Pages/EditHeaderProduct.jsx

import { useForm } from "@inertiajs/react";
import Swal from "sweetalert2";
import { Head } from "@inertiajs/react";

const EditHeaderProduct = ({ dataHeaderProduct }) => {
  // Initialize form with default values
  const { data, setData, put, processing, errors } = useForm({
    title: dataHeaderProduct?.title || "",
    description: dataHeaderProduct?.description || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!data.image_url) {
      formData.append("existing_image_url", dataHeaderProduct?.image_url || "");
    } else {
      formData.append("image_url", data.image_url);
    }

    put(`/header-product/${dataHeaderProduct?.id}`, {
      data: formData,
      ...
      onSuccess: () => {
        Swal.fire({
          ...
          window.location.href = "/header-product"; // Redirect after success
        });
      },
      onError: () => {
        Swal.fire({
  ...
  return (
    <div className="bg-white p-6">
      <Head title="Edit Header Product | PT Ratu Bio Indonesia" />
      <h1 className="mb-6 text-2xl font-bold">Edit Header Product</h1>
...
export default EditHeaderProduct;
