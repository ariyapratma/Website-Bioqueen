// resources/js/Pages/EditHeaderOrder.jsx

import { useForm } from "@inertiajs/react";
import Swal from "sweetalert2";
import { Head } from "@inertiajs/react";

const EditHeaderOrder = ({ dataHeaderOrder }) => {
  // Initialize form with default values
  const { data, setData, put, processing, errors } = useForm({
    title: dataHeaderOrder?.title || "",
    description: dataHeaderOrder?.description || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!data.image_url) {
      formData.append("existing_image_url", dataHeaderOrder?.image_url || "");
    } else {
      formData.append("image_url", data.image_url);
    }

    put(`/header-order/${dataHeaderOrder?.id}`, {
      data: formData,
      ...
      onSuccess: () => {
        Swal.fire({
          ...
          window.location.href = "/header-order"; // Redirect after success
        });
      },
      onError: () => {
        Swal.fire({
  ...
  return (
    <div className="bg-white p-6">
      <Head title="Edit Header Order | PT Ratu Bio Indonesia" />
      <h1 className="mb-6 text-2xl font-bold">Edit Header Order</h1>
...
export default EditHeaderOrder;
