// resources/js/Pages/EditHeaderMaklon.jsx

import { useForm } from "@inertiajs/react";
import Swal from "sweetalert2";
import { Head } from "@inertiajs/react";

const EditHeaderMaklon = ({ dataHeaderMaklon }) => {
  // Initialize form with default values
  const { data, setData, put, processing, errors } = useForm({
    title: dataHeaderMaklon?.title || "",
    description: dataHeaderMaklon?.description || "",
  ...
  const handleSubmit = (e) => {
    ...
    if (!data.image_url) {
      formData.append("existing_image_url", dataHeaderMaklon?.image_url || "");
    } else {
      formData.append("image_url", data.image_url);
    }

    put(`/header-maklon/${dataHeaderMaklon?.id}`, {
      data: formData,
      ...
      onSuccess: () => {
        Swal.fire({
          ...
          window.location.href = "/header-maklon"; // Redirect after success
        });
      },
      onError: () => {
        Swal.fire({
  ...
  return (
    <div className="bg-white p-6">
      ...
      <h1 className="mb-6 text-2xl font-bold">Edit Header Maklon</h1>
...
export default EditHeaderMaklon;
